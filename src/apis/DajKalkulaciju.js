...
const DajKalkulaciju =({props,token,role})=>{
    ...
    return(
        ...
        <div>
            ...
            {stranica === 0 ?  <Kalkulacije role={role} props={{token}} /> : null }
            {stranica ===2 ? <AzurirajRabat props={props} token={token} imeKolaca={props.imeKolaca} role={role} items={items} /> : null}
        </div>
    )
}
